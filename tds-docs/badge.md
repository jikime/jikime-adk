...
| typography | 't5' | "t1" | "st1" | "st2" | "st3" | "t2" | "st4" | "st5" | "st6" | "t3" | "st7" | "t4" | "st8" | "st9" | "t5" | "st10" | "t6" | "st11" | "t7" | "st12" | "st13" `Badge` 컴포넌트의 텍스트 스타일을 설정해요. 기본값은 상위 `Paragraph` 컴포넌트의 값을 따라가요. |
| style | - | StyleProp<ViewStyle> `Badge` 컴포넌트의 스타일을 변경할 때 사용해요. |
| fontWeight | 'semiBold' | "medium" | "thin" | "extralight" | "extraLight" | "light" | "normal" | "regular" | "semibold" | "semiBold" | "bold" | "extrabold" | "extraBold" | "heavy" | "black" `Badge` 컴포넌트의 텍스트 굵기를 설정해요. |
