...
| `--skip-site-flow` | Skip site-flow API integration (pages priority sync, test case creation) |
...
### Step 3.5: site-flow Page Priority Sync & Test Case Creation (AC-4)

**Condition**: Executes only when ALL of:
- `--skip-site-flow` flag is NOT set
- `site_flow.enabled` is `true` in `.migrate-config.yaml`
- User has approved the migration plan (Step 3)

**Purpose**: Sync page priorities from the approved plan to site-flow, then create verification test cases for each page.

#### Step 3.5.1: Initialize site-flow Client
...
if (!client) {
  // Graceful degradation: log warning and skip to Step 4
  console.warn('site-flow server unreachable. Skipping page sync and test case creation.');
  return;
}
```

#### Step 3.5.2: Sync Page Priorities from Migration Plan

Retrieve current pages from site-flow and update priorities based on the approved migration plan's module order:

```typescript
import { getPages, updatePage } from '../lib/site-flow';

// Get all pages registered in Phase 1 (migrate-1-analyze)
const pages = await getPages(client, site_id);

// For each page, update priority based on migration plan order
for (const module of migration_plan.modules) {
  const matchingPage = pages.find(p => p.path === module.path);
  if (matchingPage) {
    await updatePage(client, matchingPage.id, {
      metadata: {
        migrationPriority: module.priority,       // 1=first, N=last
        migrationPhase: module.phase,             // "Phase 1", "Phase 2", etc.
        estimatedEffort: module.effort_days,      // From plan estimation
        complexity: module.complexity_score        // From as_is_spec.md
      }
    });
  }
}
...
#### Step 3.5.3: Create Verification Test Cases per Page

For each page in the migration plan, create test cases that will be used in Phase 4 (verify):

```typescript
import { createTestCase } from '../lib/site-flow';

for (const module of migration_plan.modules) {
  const matchingPage = pages.find(p => p.path === module.path);
  if (!matchingPage) continue;

  // Create test case for each verification criterion
  const testCases = [
    {
      siteId: site_id,
      pageId: matchingPage.id,
      title: `[Migration] ${module.name} - Build Success`,
      description: 'Verify migrated page builds without errors (TypeScript + Lint)',
      steps: '1. Run tsc --noEmit\n2. Run eslint\n3. Run npm run build',
      expectedResult: 'Zero build errors, zero type errors, zero lint errors',
      status: 'not_tested',
      tags: ['migration', 'build-verification']
    },
    {
      siteId: site_id,
      pageId: matchingPage.id,
      title: `[Migration] ${module.name} - Visual Parity`,
      description: 'Verify migrated page visually matches the original',
      steps: '1. Navigate to migrated page\n2. Compare with original screenshot\n3. Check layout, colors, spacing',
      expectedResult: 'Visual parity with original page (>90% match)',
      status: 'not_tested',
      tags: ['migration', 'visual-verification']
    },
    {
      siteId: site_id,
      pageId: matchingPage.id,
      title: `[Migration] ${module.name} - Functional Parity`,
      description: 'Verify migrated page functionality matches the original',
      steps: '1. Test all interactive elements\n2. Verify form submissions\n3. Check navigation links',
      expectedResult: 'All original functionality preserved in migrated page',
      status: 'not_tested',
      tags: ['migration', 'functional-verification']
    }
  ];

  for (const testCase of testCases) {
    await createTestCase(client, testCase);
  }
}
```

#### Step 3.5.4: Update Configuration

```yaml
# .migrate-config.yaml updates
site_flow:
  test_case_count: 45          # Total test cases created (3 per page)
  priorities_synced_at: "2026-02-23T10:00:00Z"
  test_cases_created_at: "2026-02-23T10:00:00Z"
```

#### Step 3.5.5: Error Handling (Graceful Degradation)

- On API connection failure: Log warning, skip site-flow sync, continue to next step
- On individual test case creation failure: Log warning, queue failed request to `{artifacts_dir}/.site-flow-queue.json`, continue with remaining test cases
- On all API failures: Save full batch to queue file for retry when server recovers
...
/jikime:migrate-0-discover
        ↓ (Discovery Report + site-flow site/API key)
/jikime:migrate-1-analyze
        ↓ (as_is_spec.md + .migrate-config.yaml + site-flow pages/features)
/jikime:migrate-2-plan  ← current
        ...
        │
        ├─ [After plan approval, if site-flow enabled]
        ├─ site-flow: getPages() → Retrieve registered pages
        ├─ site-flow: updatePage() → Sync migration priorities (AC-4)
        ├─ site-flow: createTestCase() → Create verification test cases (AC-4)
        │
...
1. **Parse $ARGUMENTS**:
   ...
   - Extract `--skip-site-flow` (skip site-flow API integration)
...
9. **site-flow Page Priority Sync & Test Case Creation** (Phase 3.5):
   - IF `--skip-site-flow` is set OR `site_flow.enabled` is false: SKIP this step
   - Initialize site-flow client: `loadSiteFlowConfig()` → `createSiteFlowClient(config)`
   - IF client creation fails: Log warning, SKIP (graceful degradation per AC-7)
   - Retrieve registered pages: `getPages(client, site_id)`
   - Sync migration priorities: For each module in approved plan, `updatePage(client, pageId, { metadata: { migrationPriority, migrationPhase, estimatedEffort, complexity } })`
   - Create verification test cases (AC-4): For each page, create 3 test cases via `createTestCase(client, testCase)`:
     1. `[Migration] {pageName} - Build Success` (tsc + eslint + build)
     2. `[Migration] {pageName} - Visual Parity` (screenshot comparison >90%)
     3. `[Migration] {pageName} - Functional Parity` (interactive elements, forms, navigation)
   - Update `.migrate-config.yaml`: `site_flow.test_case_count`, `site_flow.priorities_synced_at`, `site_flow.test_cases_created_at`
   - On individual API failure: Queue to `.site-flow-queue.json` for retry
...
Version: 4.0.0
Changelog:
- v4.0.0: Added site-flow integration (SPEC-MIGRATION-001 AC-4); Added Phase 3.5 site-flow Page Priority Sync & Test Case Creation (Steps 3.5.1-3.5.5); Added --skip-site-flow option; Updated Workflow diagram with site-flow API references; Added EXECUTION DIRECTIVE step 9 for site-flow operations; Graceful degradation on connection failure (AC-7)
