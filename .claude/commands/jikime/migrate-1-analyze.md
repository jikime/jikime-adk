...
| --skip-site-flow | No | Skip site-flow API integration (analyze without registering pages/features) |
...
### Phase 4.5: site-flow Page & Feature Registration

**Condition**: Execute only when ALL of these are true:
- `--skip-site-flow` flag is NOT set
- `.migrate-config.yaml` has `site_flow.enabled: true`
- `site_flow.api_url` and `site_flow.api_key` are configured

**Purpose**: Register discovered pages and features in site-flow for tracking and verification throughout the migration workflow.

#### Step 4.5.1: Initialize site-flow Client

```python
from lib.site_flow import loadSiteFlowConfig, createSiteFlowClient, isSiteFlowEnabled

config = loadSiteFlowConfig()
if not isSiteFlowEnabled(config):
    log("site-flow integration disabled or not configured. Skipping.")
    skip_to_output()

client = createSiteFlowClient(config)
if not client:
    warn("site-flow server unreachable. Continuing without site-flow integration.")
    skip_to_output()
```

#### Step 4.5.2: Bulk Upsert Pages

Register all discovered pages (components/routes) in site-flow using `bulkUpsertPages`:

```python
from lib.site_flow import bulkUpsertPages

# Build page list from analysis results
pages = []
for route in analyzed_routes:
    page = {
        "path": route.path,                    # e.g., "/dashboard", "/settings"
        "name": route.component_name,          # e.g., "Dashboard", "Settings"
        "description": route.description,       # Auto-generated from analysis
        "pageType": classify_page_type(route),  # "static" | "dynamic" | "hybrid"
        "sourcePaths": [{
            "framework": detected_framework,    # e.g., "vue3"
            "filePath": route.file_path         # e.g., "src/views/Dashboard.vue"
        }],
        "inspectionStatus": "analyzed",         # Mark as analyzed
        "metadata": {
            "componentCount": route.component_count,
            "hasStateManagement": route.has_state,
            "hasApiCalls": route.has_api_calls,
            "complexityScore": route.complexity
        }
    }
    # Add API dependencies if dynamic page
    if route.api_dependencies:
        page["apiDependencies"] = [
            {"method": dep.method, "endpoint": dep.endpoint, "description": dep.description}
            for dep in route.api_dependencies
        ]
    pages.append(page)

# Bulk upsert - creates new pages, updates existing ones (matched by siteId + path)
result = await bulkUpsertPages(client, site_flow_config.site_id, pages)
log(f"site-flow: {result.created} pages created, {result.updated} pages updated")

# Store page ID mapping for subsequent phases
page_id_map = {page.path: page.id for page in result.pages}
```

#### Step 4.5.3: Register Features (Dynamic Pages)

For pages with API endpoints or database queries, register features in site-flow:

```python
from lib.site_flow import createFeature

for route in analyzed_routes:
    if not route.api_dependencies:
        continue  # Skip static pages

    for dep in route.api_dependencies:
        feature = {
            "siteId": site_flow_config.site_id,
            "pageId": page_id_map.get(route.path),
            "title": f"{dep.method} {dep.endpoint}",
            "description": f"API dependency: {dep.description}",
            "status": "identified",              # Will be updated in later phases
            "priority": "medium",
            ...
            "metadata": {
                ...
                "detectedAt": "migrate-1-analyze"
            }
        }
        await createFeature(client, feature)

log(f"site-flow: {feature_count} features registered for dynamic pages")
```

#### Step 4.5.4: Update Config with Page Mapping

```python
# Save page ID mapping to config for use in subsequent phases
config["site_flow"]["page_count"] = len(result.pages)
config["site_flow"]["feature_count"] = feature_count
config["site_flow"]["pages_registered_at"] = datetime.now().isoformat()
save(config)
```

#### Graceful Degradation (AC-7, AC-8)

If site-flow API calls fail during this phase:
- Log warning with error details
- Continue analysis without site-flow registration
- Save failed requests locally for retry (queue in `{artifacts_dir}/.site-flow-queue.json`)
- Inform user that site-flow registration can be retried later
...
# site-flow integration fields (added by Phase 4.5)
site_flow:
  enabled: true                       # From Step 0
  api_url: "http://localhost:3000"    # From Step 0
  api_key: "sf_..."                   # From Step 0
  site_id: "..."                      # From Step 0
  page_count: 15                      # Added by Step 1 (Phase 4.5)
  feature_count: 8                    # Added by Step 1 (Phase 4.5)
  pages_registered_at: "2026-01-23T11:30:00Z"  # Added by Step 1
```
...
10. **IF `--skip-site-flow` NOT set AND `site_flow.enabled` in config**: Execute Phase 4.5
    - Initialize site-flow client: `loadSiteFlowConfig()` → `createSiteFlowClient(config)`
    - IF client connection fails: Log warning, skip to step 11 (graceful degradation)
    - Build pages array from analysis results (path, name, pageType, sourcePaths, inspectionStatus, apiDependencies, metadata)
    - Call `bulkUpsertPages(client, site_id, pages)` to register all analyzed pages
    - FOR each dynamic page with API dependencies: Call `createFeature(client, feature)` to register features
    - Update `.migrate-config.yaml` with `site_flow.page_count`, `site_flow.feature_count`, `site_flow.pages_registered_at`
    - On any API failure: Log warning, save failed requests to `{artifacts_dir}/.site-flow-queue.json`, continue

11. **IF `--whitepaper`**: Execute Phase 5 (Whitepaper Generation)
    - Generate all 7 whitepaper documents using delegated agents
    - Apply `--lang` language setting
    - Output to `{whitepaper-output}/`

12. **Report results** to user in F.R.I.D.A.Y. format:
    - Analysis summary, site-flow registration status (if applicable), Next Step: `/jikime:migrate-2-plan`
...
Version: 3.0.0
Changelog:
- v3.0.0: Added Phase 4.5 site-flow integration (bulkUpsertPages, createFeature); Added --skip-site-flow flag; Added site_flow config fields (page_count, feature_count, pages_registered_at); Graceful degradation with .site-flow-queue.json fallback; EXECUTION DIRECTIVE updated with Phase 4.5 steps
