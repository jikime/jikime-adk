...
triggers:
  ...
  phases: ["run"]
  agents: ["refactorer", "security-auditor"]
  languages: []
...
AST-Grep is integrated into JikiME-ADK through the Tool Registry as AST_ANALYZER type in tool_registry.py, PostToolUse Hook for automatic security scanning after Write/Edit operations, and Permissions with Bash(sg:*) and Bash(ast-grep:*) auto-allowed.

### Running Scans

To scan with JikiME-ADK rules, execute sg scan with config pointing to .claude/skills/jikime-tool-ast-grep/rules/sgconfig.yml.
...
- jikime-workflow-testing: DDD integration and test pattern detection
- jikime-foundation-quality: TRUST 5 compliance and code quality gates
- jikime-domain-backend: API pattern detection and security scanning
