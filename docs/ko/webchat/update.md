...
소스가 변경된 후 webchat을 업데이트하는 방법입니다.
...
JikiME-ADK 1.8.0 이상 설치 시 가장 간단한 방법입니다.

```bash
jikime webchat install
```

최신 webchat 소스를 다운로드하고 `pnpm install` + `pnpm build`를 자동 실행합니다.
기존 `node_modules`와 `.next`는 보존됩니다.
